...
{	
		...
		$data = json_decode(file_get_contents("php://input")); 
		 ...
		 $InvoiceDetatilPkId = json_decode($_REQUEST['InvoiceDetatilPkId']);
    ...
    $Ordrchk=array();	 
    $Qtychk=array();	 
  	foreach ($InvoiceDetatilPkId as $key => $value) {
	 	# code...
	 	if($InvoiceDetatilPkId[$key]!="")
    	{
    		$Ordrchk[]=1;	
    	}
    	else
    	...
    	else
    	{
		 ...
		 $delete_status = 1;
		if($OrderId!="" && $InvoiceId!=""  && $customerid!="" && $sum!="")
		{
			$check = 0;
			$Errors .= "";
		...
		else 
		{
			...
			$query = $dbConnection->prepare("INSERT INTO Packages (PackageId,InvoiceId,PkgDate,InternalNotes,CreatedTime) VALUES (?,?,?,?,?)");
			$query->execute(array($PackageId,$InvoiceId,$entrydate,$internalnotes,$DATEIME));
			...
			foreach ($InvoiceDetatilPkId as $key => $value) {
				...
				if($qtytopack1>0)
				{
					...
					$query1 = $dbConnection->prepare("INSERT INTO PackageDetails (PackageId_Packages,PkId_InvoiceDetails,Packed,CreatedTime) VALUES (?,?,?,?)");
					$query1->execute(array($PackageId,$InvoiceDetatilPkId[$key],$qtytopack[$key],$DATEIME));

					$query2 = $dbConnection->prepare("UPDATE InvoiceDetails SET PackedQty=? WHERE PkId=? AND InvoiceId_Invoices=?");
					$query2->execute(array($NewQty1,$InvoiceDetatilPkId[$key],$InvoiceId));					
				}
			}

			if(($ordersum-$packedsum)==$sum)
			{
				//$OrderStatus=3//confirmed
				//PackageStatus=2 //Full
				$query2 = $dbConnection->prepare("UPDATE Invoices SET PackageStatus=? WHERE InvoiceId=?");
			...
			else
			{
				...
				$query2 = $dbConnection->prepare("UPDATE Invoices SET PackageStatus=? WHERE InvoiceId=?");
				$query2->execute(array('1',$InvoiceId));
			}
