...
{		
	...
	$searchslitid = $data->searchslitid;
	...
	if($searchslitid!="")
	{
		$final .= "Slitting.SlitId='$searchslitid' AND ";
	}
	if($searchfromdate!="" && $searchtodate!="" && $searchfromdate>"1970-01-01" && $searchtodate>"1970-01-01" && $searchtype=="From & To Date")
	{
		$final .= "(Slitting.SlitDate BETWEEN '$searchfromdate' AND '$searchtodate') AND ";
	}
	if($itemname!="")
	{
		$final .= "SlittingRolls.ProductId_ProductMaster='$itemname' AND ";
	}
	...
	$query = $dbConnection->prepare("SELECT Slitting.* FROM Slitting LEFT JOIN SlittingRolls ON Slitting.SlitId=SlittingRolls.SlitId_Slitting WHERE Slitting.DeleteStatus=? AND $qry GROUP BY Slitting.SlitId ORDER BY Slitting.PkId DESC LIMIT $first,$itemsPerPage");
	...
	{	
		...
		if($searchslitid!="")
		{
			$sql = $dbConnection->prepare("SELECT COUNT(Slitting.PkId) AS Total FROM Slitting WHERE Slitting.DeleteStatus=? AND Slitting.SlitId='$searchslitid' GROUP BY Slitting.SlitId ORDER BY Slitting.PkId DESC");
		...
		if($searchfromdate!="" && $searchtodate!="" && $searchfromdate!="1970-01-01" && $searchtodate!="1970-01-01" && $searchtype=="From & To Date")
		{
			$sql = $dbConnection->prepare("SELECT COUNT(PkId) AS Total FROM Slitting WHERE DeleteStatus=? AND SlitDate BETWEEN '$searchfromdate' AND '$searchtodate' ORDER BY PkId DESC");
			$sql->execute(array($delete_status));
			$row4 = $sql->fetch();
			$Total = $row4['Total'];
		}
		if($itemname!="")
		{
			...
			$row4 = $sql->fetch();
			$Total =  $row4['Total'];*/
			$sql = $dbConnection->prepare("SELECT Slitting.* FROM Slitting LEFT JOIN SlittingRolls ON Slitting.SlitId=SlittingRolls.SlitId_Slitting WHERE Slitting.DeleteStatus=? AND $qry GROUP BY Slitting.SlitId ORDER BY Slitting.PkId DESC");
			$sql->execute(array($delete_status));
