...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;	
	$scope.FormList = true;	
	$scope.pagetitle = "Account of Finished Products";
...
//$scope.DeliveryArray = ['Transport','Courier','Auto','Rikshaw'];
	...
	{
		$http.get("load-spare-ledger.php")
...
$scope.LedgerArr = [];
$scope.GetProductData= function(ProductId,ProductName)
{
	$scope.ProductName=ProductName;
	$http.post("get-product-ledger.php",{'ProductId':ProductId,'ProductName':ProductName})
	.then(function successCallback(response)
	{
		var data = response.data;
