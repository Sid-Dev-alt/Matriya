...
$scope.InvoiceArr=[];
$scope.GetallInvioces = function(customerid)
{
	 $http.post("Get-customer-invoices.php",{'customerid': customerid})
    .then(function successCallback(response)
		{
			var data = response.data;
			if(data=="NoData")
...
//onload -1
$scope.GetInvoice = function(InvoicePkId)
{
	if(InvoicePkId==undefined)
	{
		$scope.GetListData();
	}
	else
	{
		...
		$scope.GetPaymentTerms();
		$scope.GetInventory();
		$scope.GetInvoiceInfo(InvoicePkId);
	}
}
//onload -2
$scope.GetInvoiceInfo = function(InvoicePkId)
{
	$http.post("Get-invoice-pkgs.php",{'PkId': InvoicePkId})
		.then(function successCallback(response)
		{
			var data = response.data;			
			if(data=="NoData")
			...
			{
				$scope.GetallInvioces(data['CustomerId_CustomerMaster']);
				$scope.InvoicePkId =data['PkId'];				
...
$scope.GetListData= function()
	{
		$http.get("load-invoice-packages.php")
		.then(function successCallback(response)
		{
			var data = response.data; 
 ...
 $scope.onCSelect = function ($item, $model, $label) {
    ...
    $scope.$model = $model;
    $scope.$label = $label;
    $scope.GetallInvioces($item.CustomerId);
};
...
{
	window.location.href="invoice-package.php";
}
...
$scope.ChangeQty = function(quantity,qtytopack,index)
{
	...
	$scope.AddCategoryData = function ()
	{
        ...
        var InvoiceDetatilPkId=new Array();
	...
	for(i=0;i<$scope.BatchArr.length;i++)
	    {
		 InvoiceDetatilPkId.push($scope.BatchArr[i].InvoiceDetatilPkId);
		...
		//alert($scope.OrderId);
			$scope.FileErr="";
			$http.post('add-invoice-package-process.php', fd, {transformRequest: angular.identity,headers: {'Content-Type': undefined},
            params: {
				...
				'InvoiceDetatilPkId':JSON.stringify(InvoiceDetatilPkId),
			...
			.then(function successCallback(response)
			{
				...
				if(data=="Success")
				{
					...
					swal({title: "Added",
							     ...
							     type: "success",
							     timer: 1000 
						},function () {window.location.href="invoice-package.php";})
				}
				else
				{
