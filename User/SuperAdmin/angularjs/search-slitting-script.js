...
$scope.vm.dtOptions = DTOptionsBuilder.newOptions()
  ...
  mindt.setDate(mindt.getDate() -90);
    ...
    $scope.doubleopen = function($event,datepicker) {
      $event.preventDefault();
      $event.stopPropagation();
      $scope.opened[datepicker] = true;
    };
    
    $scope.tribleopen = function($event,datepicker) {
      $event.preventDefault();
      $event.stopPropagation();
      $scope.opened[datepicker] = true;
    };
...
$scope.getData = function(pageno){ 
    ...
    // This would fetch the data on page change.
        ...
        $http.post("load-search-slitting.php",{'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno,'searchslitid': $scope.searchslitid, 'searchfromdate': $scope.searchfromdate,'searchtodate':$scope.searchtodate,'itemname':$scope.itemname.ProductId,'searchtype':$scope.searchtype})
          .then(function successCallback(response)
        {
            var data = response.data;
