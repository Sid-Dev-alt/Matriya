...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, $window, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;
	$scope.FormList = true;
	$scope.pagetitle = "List of Printings";
...
$scope.GetBlc = function(){
$scope.balance = ($scope.rollno.SplitQty-$scope.consumeqty).toFixed(3) || 0.000;
}
...
$scope.ProductArray = [];
	$scope.GetProducts = function(rawcategory)
	{
		...
		$scope.rollno="";
		$scope.rollavlqty="";
        $http.post('get-category-products.php',{'CatPkId': rawcategory.PkId})
        .then(function successCallback(response)
		{
			var data = response.data;
        	if(data=="NoData")
...
//     $scope.$model = $model;
//     $scope.$label = $label;
//     $scope.GetRolls($item.ProductId);
//     //$scope.RollArr = $item.data2;
// };
$scope.GetRolls = function(productsize)
{
	//alert();
    $http.post('get-raw-rolls.php',{'ProductId': productsize.ProductId})
    .then(function successCallback(response)
	{
		var data = response.data;
		if(data=="NoData")
...
//$scope.DeliveryArray = ['Transport','Courier','Auto','Rikshaw'];
	...
	$scope.EditOrder = function(FormPkId,PrintDate,PkId_JobMaster,JobName,PkId_Category,ProductId_ProductMaster,ProductName,PkId_RawPurchaseMasterDetails,RollNo,ConsumeQty,Balance,Waste,PrintOutput,Remarks)
	{		
		$scope.GetCategories();
		$scope.GetJobs();
		$scope.GetProducts(PkId_Category);
		$scope.GetRolls(ProductId_ProductMaster);
		$scope.GetRollQty(PkId_RawPurchaseMasterDetails);
		$scope.FormList = false;
		$scope.FormAdd = true;
		$scope.pagetitle = "Edit Printing";
		$scope.FormPkId=FormPkId;
		$scope.entrydate=new Date(PrintDate);
		$scope.Oldjobid=PkId_JobMaster;
		$scope.jobname=JobName;
		
		$scope.rawcategory=PkId_Category;
		$scope.oldproductid=ProductId_ProductMaster;
		$scope.productsize=ProductName;
		$scope.oldrollno = PkId_RawPurchaseMasterDetails;
		$scope.rollno = PkId_RawPurchaseMasterDetails;
		//$scope.rollno=RollNo;
		$scope.oldconsumeqty=ConsumeQty;
		$scope.consumeqty=ConsumeQty;
	...
	$scope.AddCategoryData = function ()
	{
		...
		{
			swal("STOP", "Please Select Job", "error");
			$timeout(function () { $scope.submitted = false;}, 3000);
		}
		else if($scope.rawcategory==undefined)
		{
			swal("STOP", "Please Select Category", "error");
			$timeout(function () { $scope.submitted = false;}, 3000);
		}
		else if($scope.productsize==undefined)
		{
			swal("STOP", "Please Select Product Size", "error");
			$timeout(function () { $scope.submitted = false;}, 3000);
		}
		else if($scope.rollno==undefined)
		{
			swal("STOP", "Please Select Roll No", "error");
			$timeout(function () { $scope.submitted = false;}, 3000);
		}
		...
		{
			$scope.FileErr="";
			$http.post('add-printing-process.php',
    ...
    //         params:
            {
				...
				'jobname':$scope.jobname.JobName,
				//'rawcategory':$scope.rawcategory,
				'rawcategory':$scope.rawcategory.PkId,
				'CategoryName':$scope.rawcategory.CategoryName,

				// 'productid':$scope.productid,
				// 'productname':$scope.productsize,

				'productid':$scope.productsize.ProductId,
				'productname':$scope.productsize.ProductName,

				'rollno':$scope.rollno.PkId,
				'rollname':$scope.rollno.RollNo,
				'rollavlqty':$scope.rollno.RollAvlQty,

				'consumeqty':$scope.consumeqty,
			...
			.then(function successCallback(response)
			{
				...
				if(data=="Success")
				{
					...
					swal({title: "Added",
							     text: "Printing created successfully",
							     type: "success",
							     timer: 2000
		...
		{
			$http.post('update-printing-process.php',
			 // fd, {transformRequest: angular.identity,headers: {'Content-Type': undefined},
    //         params:
            {
				...
				'rawcategory':$scope.rawcategory,
				'oldproductid':$scope.oldproductid,
				'productid':$scope.productid,
				'oldrollno':$scope.oldrollno,
				'rollno':$scope.rollno,
				'rollavlqty':$scope.rollavlqty,
				'oldconsumeqty':$scope.oldconsumeqty,
				'consumeqty':$scope.consumeqty,
				'balance':$scope.balance,
				'wastage':$scope.wastage,
				'printoutput':$scope.printoutput,
			...
			.then(function successCallback(response)
			{
				...
				if(data=="Success")
				{
					...
					swal({title: "Updated",
							     text: "Printing details updated successfully",
							     type: "success",
							     timer: 2000
						},function () {window.location.reload();})
				}
