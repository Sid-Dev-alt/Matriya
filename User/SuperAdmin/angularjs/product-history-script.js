...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;	
	$scope.FormList = true;	
	$scope.pagetitle = "History of Product";
...
$scope.BatchArr = [{}];
    ...
    $scope.invtype = "Finished";
$scope.GetInventory = function(invtype)
{
	$http.post("load-inventory-wise-materials.php",{'invtype': invtype})
...
//$scope.DeliveryArray = ['Transport','Courier','Auto','Rikshaw'];
	...
	{
		$http.post("load-history-materials.php",{
		...
		{
			var data = response.data;	
				$scope.ProducedArr = data[0]['data1'];
				$scope.DispatchArr = data[0]['data2'];
				$scope.RawDataArr = data[0]['data3'];
				//$scope.FormList = true;
		});
	}
