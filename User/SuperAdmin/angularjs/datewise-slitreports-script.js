...
$scope.GetRollData= function(UniqueRollNo)
	...
	$scope.GetRollDataDetail= function(RollNo)
	{
		$scope.loading = true;
		//$scope.mypageno = pageno;
...
$scope.itemsPerPage = 10;
	$scope.GetListData= function(pageno)
	{
	    ...
	    {
			$http.post("load-datewise-slitting-data.php", {'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno, 'fromdate': $scope.fromdate, 'todate': $scope.todate})
			.then(function successCallback(response)
			{
				var data = response.data; 
