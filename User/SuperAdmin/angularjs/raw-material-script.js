...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;	
	$scope.FormList = true;	
	$scope.pagetitle = "Account of Raw materials";
...
//$scope.DeliveryArray = ['Transport','Courier','Auto','Rikshaw'];
	...
	{
		$http.get("load-raw-materials.php")
...
$scope.GetProductData= function(ProductId,ProductName,Micron,Size,Unit)
{
	$scope.ProductName=ProductName;
	$scope.Micron=Micron;
	$scope.Size=Size;
	$scope.Unit=Unit;
	$http.post("get-raw-ledger.php",{'ProductId':ProductId,'ProductName':ProductName,'Micron':Micron,'Size':Size})
	.then(function successCallback(response)
	{
		var data = response.data;
		$scope.LedgerArr = data;
	});
}
