...
{		
	...
	if($customerid!="")
	{
		...
		$query = $dbConnection->prepare("SELECT * FROM Invoices WHERE DeleteStatus=? AND PackageStatus!=? AND PaymentStatus=? AND CustomerId_CustomerMaster=?");
		$query->execute(array($delete_status,'2','0',$customerid));
