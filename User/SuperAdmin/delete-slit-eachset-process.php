...
{	
		...
		//set Invoice delete
			$query39 = $dbConnection->prepare("SELECT IsSplitQty,Status FROM InventoryMaster WHERE UniqueRollNo=?");
			$query39->execute(array($RollNo));
			$row = $query39->fetch();
			$IsSplitQty = $row['IsSplitQty'];
			$Status = $row['Status'];
			if($IsSplitQty=="1")
			{
				$check=1;
				$Errors .= "You cannot delete this item. item already split.\n";		
			}
			if($Status=="0")
			{
				$check=1;
				$Errors .= "You cannot delete this item. item already dispatched.\n";		
			}
		...
		if($check==0)
		{
			...
			$query133 = $dbConnection->prepare("UPDATE SlittingRolls SET DeleteStatus=? WHERE PkId=?");
			$query133->execute(array('0',$EntryPkId));	

			//update fresh quantity
			$query11 = $dbConnection->prepare("UPDATE InventoryMaster SET DeleteStatus=? WHERE UniqueRollNo=?");
			$query11->execute(array('0',$RollNo));
			...
			$query = $dbConnection->prepare("SELECT * FROM Slitting WHERE SlitId=? AND DeleteStatus=?");
			$query->execute(array($SlitId,$delete_status));
