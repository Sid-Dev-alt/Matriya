...
<html lang="en">
	<head>
    	...
    	<script src="angularjs/raw-purchase-one-script.js"></script>
    	<style type="text/css">
    		.new-row {
                clear: left;
            }
    					...
    					<div class="row" ng-show="FormList">
    					    <div class="col-xs-12 col-sm-12 col-md-12">
								<div class="col-xs-12 col-sm-12 col-md-6">
									<input type="text" ng-model="searchpro" class="form-control" placeholder="Search"> 
								</div>
								<div class="col-xs-12 col-sm-12 col-md-6">
									<button class="btn btn-sm" ng-click="getData(pageno)">Go</button>
								</div>
							</div>
    						<div class="col-xs-12">
    							<div class="row">
    								...
    								<div class="row">
    									<div class="col-xs-6">
    										<div id="dynamic-table_filter" class="dataTables_filter pull-left">
    											<input type="text" ng-model="search" class="form-control" placeholder="Filter"> &nbsp; Showing {{mypageno*itemsPerPage-itemsPerPage+1}}-{{mypageno*itemsPerPage}} of {{total_count}}
    										</div>
    									</div>
    								</div>
    								<table id="dynamic-table" class="table table-bordered table-striped table-condensed cf">
    									<thead class="cf">
    										<tr>
    											<th ng-click="sort('Purchase')">Purchase Id <span class="glyphicon sort-icon" ng-show="sortKey=='Purchase'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}"></span></th>
    											<th ng-click="sort('Date')">Date <span class="glyphicon sort-icon" ng-show="sortKey=='Date'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}"></span></th>
    											<th ng-click="sort('Party')">Party Name <span class="glyphicon sort-icon" ng-show="sortKey=='Party'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}"></span></th>
    									...
    									<tbody>
    										<tr dir-paginate="cat in pagedItems|orderBy:sortKey:reverse|filter:search|itemsPerPage:5" total-items="{{total_count}}" current-page="mypageno">
    											...
    											<td data-title="Party Name">{{cat.VendorName}}</td>
    											<td data-title="Item Details">
    												<table id="dynamic-table" class="table table-bordered table-striped table-condensed cf" >
        												...
        												<tbody>
            												<tr ng-repeat="sp in cat.data2|filter:search|orderBy:sortKey1:reverse1">
            													<td data-title="Item">{{sp.TotalProductName}}<!--<br />{{sp.Micron}} {{sp.ProductName}} {{sp.ProductSize}} MM --></td>
            													...
            													<td data-title="Print">
            														<a href="generate-purchase-barcode.php?Id={{sp.EntryPkId}}" class="btn btn-xs btn-danger" >Print &nbsp;
    									...
    									<tfoot>
    										<tr>
    											<td colspan="8">
    												<ul class="pagination">
    													<li>
    													    <dir-pagination-controls max-size="5" direction-links="true" boundary-links="true" on-page-change="getData(newPageNumber)"></dir-pagination-controls>
    											        </li>
    											    </ul>
    											</td>
    										</tr>
    							...
    							<form class="form-horizontal" role="form" id="AddCategoryForm" name="AddCategoryForm" autocomplete="off" enctype="multipart/form-data" novalidate="" data-ng-submit="AddCategoryData()">
                                        ...
                                        <table class="table table-striped table-bordered" >
                                            ...
                                            <tbody>
                                                <tr ng-repeat="bat in BatchArr" >
                                                        ...
                                                        <div class="form-group">
                                                            <div class="col-sm-12">
                                                                <div class="input-group">
                                                                    ...
                                                                    <span class="input-group-addon">{{bat.Unit}}</span>
                                                                </div>
                                                    ...
                                                    <td data-title="ACTION">
                                                        <a ng-if="bat.DeleteStatus=='0'" href="#" class="orange" ng-click="removeRow($index);GetProducts()"><i class="ace-icon fa fa-remove bigger-130"></i></a>
                                                        <a ng-if="bat.DeleteStatus=='1'" href="#" class="orange" ng-click="Delete(bat.EntryPkId,bat.productname,bat.IsSplitQty,bat.Isinvoiced);GetProducts()"><i class="ace-icon fa fa-trash bigger-130"></i></a>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    ...
                                    <div class="clearfix form-actions" >
                                        <div class="col-md-offset-3 col-md-9">
                                            <button class="btn btn-info" type="submit"  ng-if="pagetitle!='Edit Production'"><i class="ace-icon fa fa-check bigger-110"></i>SAVE</button>&nbsp; &nbsp;<button class="btn btn-inverse" type="button" data-ng-click="GotoList()"><i class="ace-icon fa fa-close bigger-110"></i>CANCEL</button>
                                        </div>
                                    </div>
    							</form>
    						</div>
        									...
        									<div class="card">
            									...
            									<div class="card-body" ng-if="result=='OK'">
            										...
            										<ul>
            											...
            											<li ng-if="IsInvoice=='1'">Roll has been dispatched to <span class="text-success">{{CustomerNameModal}}</span> on <b>{{InvoiceDate | date: 'd MMM y'}} </b></li>
            										</ul>
            									</div>
        									</div>
        								</div>
