...
<html lang="en">
	<head>
    	...
    	<script src="angularjs/invoice-package-script.js"></script>
	</head>	

	<body class="no-skin" data-ng-cloak data-ng-app="CategoryModule" data-ng-controller="CategoryController" data-ng-init="GetInvoice(<?php echo $InvoicePkId;?>)">
									...
									<table id="dynamic-table" class="table table-bordered table-striped table-condensed cf"  datatable="ng" dt-options="vm.dtOptions" dt-column-defs="vm.dtColumnDefs">
										...
										<tbody >
											<tr  ng-repeat="cat in pagedItems">
												...
												<td data-title="Date">{{cat.PkgDate | date: 'd MMM y'}}</td>
												...
												<td data-title="Package Status">
													<div ng-if="cat.PackageStatus=='1' || cat.PackageStatus=='0'">
														<p>Not shipped</p>
														<p><a href="shipping.php?Id={{cat.PkId}}"><span class="label label-info arrowed-right arrowed-in" ng-click="SetasShip()">Create Delivery challan</span></a></p>
													</div>
													<span ng-if="cat.PackageStatus=='2'"> Shipped</span>
													<span ng-if="cat.PackageStatus=='3'">Delivered</span>
												</td>
							...
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
								 <form class="form-horizontal" role="form" id="AddCategoryForm" name="AddCategoryForm" autocomplete="off" enctype="multipart/form-data" novalidate="" data-ng-submit="AddCategoryData()">
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="Product Id"> Invoice Number <span class="error">*</span></label>
										<div class="col-sm-6">
											<select type="text" id="InvoicePkId" placeholder="" class="form-control" name="InvoicePkId" required  data-ng-model="InvoicePkId" ng-change="GetInvoiceInfo(InvoicePkId)" convert-to-number>
												<option value="">Select</option>
												<option ng-repeat="inv in InvoiceArr" value="{{inv.PkId}}">{{inv.InvoiceId}}</option>
											</select>
												<div class="error" data-ng-show="submitted || AddCategoryForm.InvoicePkId.$dirty && AddCategoryForm.InvoicePkId.$invalid">
													<small class="error" data-ng-show="AddCategoryForm.InvoicePkId.$error.required">Invoice Id is required.</small>
												</div>
										</div>
									</div>
									...
									<tbody>
										<tr ng-repeat="bat in BatchArr">
											...
											<div class="form-group">
												<div class="col-sm-12">
													<input type="hidden" value="{{bat.InvoiceDetatilPkId}}">
												{{bat.product}}
											...
											<td data-title="Invoice Qty" class="center">
												<div class="form-group">
												<div class="col-sm-12">
													{{bat.quantity}}
											...
											<td data-title="Status" class="center" >
												<p>{{bat.PackedQty}} Packed </p>
												<p>{{bat.quantity-bat.PackedQty}} Quantity to pack</p>
											</td>
								...
								<div class="form-group" style="display: none">
									<label class="col-sm-7 control-label no-padding-right" for="Choose ">Qty to pack </label>
									<div class="col-sm-3">&nbsp;</div>
									<div class="col-sm-2">
