...
{		
	...
	$query3 = $dbConnection->prepare("SELECT PkId,PkId_RawPurchaseMasterDetails,UniqueRollNo,Quantity,ProductId_ProductMaster,ProductSize,Remarks FROM InventoryMaster WHERE UniqueRollNo=? AND DeleteStatus=? AND IsSplitQty=? AND Quantity>0 AND Status=?" );
	$query3->execute(array($barcodeInput,$delete_status,'0',$delete_status));
	if($query3->rowCount()>0)
	{
		...
		$row1 = $query1->fetch();
		$RawPurchaseId_RawPurchaseMaster = $row1['RRawPurchaseId_RawPurchaseMaster'];
		...
		$query3 = $dbConnection->prepare("SELECT ProductId,ProductName,Unit,Size,Micron FROM ProductMaster WHERE ProductId=?");
		...
		$data1 = array("InvPkId"=>$InvPkId,"PkId_RawPurchaseMasterDetails"=>$PkId_RawPurchaseMasterDetails,"ProductId"=>$ProductId_ProductMaster,"ProductName"=>$ProductName,"Unit"=>$Unit,"Size"=>$ProductSize,"Remarks"=>$Remarks,"Micron"=>$Micron,"GoDownName"=>$GoDownName,"UniqueRollNo"=>$UniqueRollNo,"TotalName"=>$TotalName,"quantity"=>$quantity);
