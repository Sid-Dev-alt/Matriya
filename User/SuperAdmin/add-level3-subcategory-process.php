...
{	
		...
		if($category!="" && $subcatname!="" && $lvl2subcatname!="" && $lvl3subcatname!="")
		{
			...
			$query = $dbConnection->prepare("INSERT INTO Level3SubCategoryMaster (PkId_CategoryMaster,PkId_SubCategoryMaster,PkId_Level2SubCategoryMaster,Level3SCName,CreatedTime) VALUES (?,?,?,?,?)");
			$query->execute(array($category,$subcatname,$lvl2subcatname,$lvl3subcatname,$DATEIME));
			$PkId = $dbConnection->lastInsertId();

			$Post = "$SAId Created Level3SubCategoryMaster (Level-3 SubCategoryName: $lvl3subcatname) as on $DATEIME";
