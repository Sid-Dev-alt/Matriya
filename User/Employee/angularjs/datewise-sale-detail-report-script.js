...
$scope.itemsPerPage = 10;	
	$scope.GetListData= function(pageno)
	{
	    ...
	    {
			$http.post("load-datewise-sale-detail-report-data.php", {'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno, 'fromdate': $scope.fromdate, 'todate': $scope.todate})
	 ...
	 //    {
	 	//'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno, , 'ProductSize': $scope.itemsize.ProductSize
          ...
          $scope.result = "OK";
            ...
            $scope.GetListData($scope.pageno);
				 }
			}, function errorCallback(response) {
	    		// called asynchronously if an error occurs
			    // or server returns response with an error status.
...
$scope.SetMinusQty = function(quantity,setquantity,index)
{
	...
	{		
		$("#myModal").modal('hide');
		//window.location.href = "datewise-sale-detail-reports.php";
	}
});
