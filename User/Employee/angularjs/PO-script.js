...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;	
	$scope.FormList = true;	
	$scope.pagetitle = "List of Raw Purchases";
...
$scope.BatchArr = [{InventoryType:'Raw',price:0}];

    $scope.AddMore = function() 
    {
      $scope.BatchArr.push({InventoryType:'Raw',price:0});
    }
...
{
	$scope.pagetitle = "Add Raw Purchase";
	$scope.FormAdd = true;	
...
$scope.onInvSelect = function ($item, $model, $label, index) {
    ...
    $scope.BatchArr[index].InventoryType = $item.InventoryType;
    //$scope.BatchArr[index].InventoryType = $item.SKU;
...
$scope.InventoryArray = [];
	...
	{
		$http.get("get-raw-spare-materials.php")
		.then(function successCallback(response)
		{
			var data = response.data;  
			if(data=="NoData")
...
// 	}
	...
	$scope.AddCategoryData = function ()
	{
        ...
        var OldProductId=new Array();
        var ProductId=new Array();
		var product=new Array();
		var Oldquantity=new Array();
	...
	for(i=0;i<$scope.BatchArr.length;i++)
	    ...
	    EntryPkId.push($scope.BatchArr[i].EntryPkId);	
		 OldProductId.push($scope.BatchArr[i].OldProductId);
		 ProductId.push($scope.BatchArr[i].ProductId);

		 product.push($scope.BatchArr[i].product);

		 Oldquantity.push($scope.BatchArr[i].Oldquantity);
		...
		{
			$scope.FileErr="";
			$http.post('add-PO-process.php', fd, {transformRequest: angular.identity,headers: {'Content-Type': undefined},
            params: {
				...
				//'OldProductId':JSON.stringify(OldProductId),
				'ProductId':JSON.stringify(ProductId),
				'product':JSON.stringify(product),
				'quantity':JSON.stringify(quantity),
				//'Oldquantity':JSON.stringify(Oldquantity),
				'price':JSON.stringify(price),
 ...
 //      if (isConfirm) {
	...
	// 		$http.post('delete-PO-process.php',
	//             {
	// 				'PkId':PkId,
	// 				'POrderId':POrderId,
	// 			})
