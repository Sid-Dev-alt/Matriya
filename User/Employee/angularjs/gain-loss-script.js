...
$scope.itemsPerPage = 10;
	$scope.GetListData= function(pageno)
	{
	    ...
	    {
	 	//'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno, , 'ProductSize': $scope.itemsize.ProductSize
			$http.post("load-itemwise-gainloss-report-data.php", {'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno, 'fromdate': $scope.fromdate, 'todate': $scope.todate,'Micron': $scope.itemnname.Micron, 'ProductName': $scope.itemnname.ProductName})
			.then(function successCallback(response)
			{
				var data = response.data; 
...
$scope.calculateSum = function () {
  ...
  if ($scope.pagedItems !== undefined) {
	...
	{		
		window.location.href = "gain-loss-report.php";
	}
});
