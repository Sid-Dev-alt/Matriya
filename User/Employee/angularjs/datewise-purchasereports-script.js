...
$scope.itemsPerPage = 10;
	$scope.GetListData= function(pageno)
	{
	    ...
	    {
			$http.post("load-datewise-purchasereport-data.php", {'itemsPerPage': $scope.itemsPerPage, 'pagenumber': pageno, 'fromdate': $scope.fromdate, 'todate': $scope.todate})
			.then(function successCallback(response)
			{
				var data = response.data; 
...
$scope.SetMinusQty = function(quantity,setquantity,index)
{
	...
	{		
		window.location.href = "datewise-purchase-reports.php";
	}
});
