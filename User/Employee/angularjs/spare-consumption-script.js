...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;	
	$scope.FormList = true;	
	$scope.pagetitle = "List of spares consumed";
...
$scope.DeliveryArray = ['Transport','Courier','Auto','Rikshaw'];
	...
	{
		$http.get("load-spare-consumptions.php")
...
$scope.onInvSelect = function ($item, $model, $label, index) {
    ...
    $scope.BatchArr[index].AvlQty = $item.AvlQty;
...
$scope.InventoryArray = [];
	$scope.GetSpares= function()
	{
		$http.get("get-spare-products.php")
		.then(function successCallback(response)
		{
			var data = response.data;  
			if(data=="NoData")
...
$scope.onEditInvSelect = function($item, $model, $label)
{
	$scope.$item = $item;
    $scope.ProductId = $item.ProductId;
    $scope.product = $item.PName;
    $scope.price = $item.SalesPrice;
    $scope.AvlQty = $item.AvlQty;
    $scope.SKU = $item.SKU;
    $scope.$model = $model;
    $scope.$label = $label;
}
	$scope.EditOrder = function(PkId,CreatedDate,ProductId,ProductName,TotalConsumption,AvlQuantity)
	{
		$scope.FormList = false;
		$scope.FormAdd = false;
		$scope.FormEdit = true;
		$scope.pagetitle = "Edit Spare Consumption";
		//$scope.GetPaymentTerms();
		//$scope.GetVendors();
		$scope.GetSpares();
		$scope.FormPkId = PkId; 
		$scope.entrydate= new Date(CreatedDate);
		$scope.OldProductId= ProductId;
		$scope.product= ProductName;
		$scope.Oldquantity= TotalConsumption;
		$scope.editquantity= TotalConsumption;
		$scope.AvlQty= AvlQuantity;
		//$scope.EditChkQty()
	}


	$scope.ChkQty = function(quantity,AvlQty,index)
	{
		if(quantity>AvlQty)
		{
			$scope.BatchArr[index].quantity = AvlQty;
		}
	}

		$scope.EditChkQty = function(editquantity,AvlQty)
	{
		if(quantity>AvlQty)
		{
			$scope.editquantity = AvlQty;
		}
	}
 ...
 //         fd.append('file', file);	
	...
	for(i=0;i<$scope.BatchArr.length;i++)
		...
		// {
			$scope.FileErr="";
			$http.post('spare-consumption-process.php',{
			...
			.then(function successCallback(response)
			{
				...
				if(data=="Success")
				{
					...
					swal({title: "Added",
							     text: "Spare Consumption created successfully",
							     type: "success",
							     timer: 2000 
	...
	$scope.UpdateCategoryData = function ()
	{
		$scope.submitted = true;		
			$scope.FileErr="";
			//alert($scope.editquantity);
			$http.post('update-spare-consumption-process.php',{
			...
			.then(function successCallback(response)
			{
				...
				if(data=="Success")
				{
					...
					swal({title: "Added",
							     text: "Spare Consumption updated successfully",
							     type: "success",
							     timer: 2000 
						},function () {window.location.reload();})
				}
	...
	{		
		window.location.href = "spare-consumption.php";
	}
