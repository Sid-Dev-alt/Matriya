...
$scope.Gotoback = function()
{
  ...
  {
    $http.get("Get-Warehouse-Items.php")
...
$scope.ProductArray =[];
$scope.GetProductList= function(location,index)
  {
...
$(".transferto").droppable({
    ...
    var ele = ui.draggable.attr('id').split("&");
    var quantity=  Number(ui.draggable.find("input[name=quantity]").val());
    var AvailableQty=  Number(ui.draggable.find("input[name=AvailableQty]").val());
    ...
    {
      alert("Quantity should be less than or equal to "+ AvailableQty);
    }
    else
    {
      swal({
  ...
  function(isConfirm) {
    if (isConfirm) {
        ...
        $scope.TransferFrom = ele[0];
        $scope.sku=ele[1];
         $http.post('add-drag-transfer-process.php',{
        'location': $scope.TransferFrom,
        'sku': $scope.sku,
        'pquantity': quantity,
        ...
        if(data=="Success")
        {
         // window.location.reload();
          swal({title: "Success",
                     ...
                     type: "success",
                     timer: 1500 
              },function () {window.location.href="drogtransfer.php"})
        }
        else
        {
