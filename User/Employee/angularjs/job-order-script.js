...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, $window, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;
	$scope.FormList = true;
	$scope.pagetitle = "List of Orders";
...
//     $scope.$model = $model;
//     $scope.$label = $label;
//     $scope.GetLamiation($item.PkId);
// };

$scope.SlitInputArr = [];
	$scope.GetLamiation = function(jobname)
	{
        $http.post('get-lamination-value.php',{'JobPkId': jobname})
        .then(function successCallback(response)
		{
			var data = response.data;
        	if(data=="NoData")
...
// };
// $scope.GetRollQty = function(slitinput)
// {
// 	//alert();
//     $http.post('get-lamination-rollqty.php',{'slitinput': slitinput})
//     .then(function successCallback(response)
// 	{
// 		var data = response.data;
...
//$scope.DeliveryArray = ['Transport','Courier','Auto','Rikshaw'];
	...
	$scope.EditOrder = function(FormPkId,SlitDate,PkId_JobMaster,JobName,CutSize,SlitInput,Balance,NoofRolls,LaminationWastage,TrimWastage,FinalOutput,Remarks)
	{		
		...
		$scope.cuttingsize=CutSize;
		$scope.slitinput=SlitInput;
		$scope.blcofjob=Balance;
		$scope.noofrolls=NoofRolls;
		$scope.laminationwaste=LaminationWastage;
		$scope.trimwaste=TrimWastage;
		$scope.finaloutput=FinalOutput;
	...
	$scope.AddCategoryData = function ()
	{
		...
		{
			$scope.FileErr="";
			$http.post('add-job-order-process.php',
    ...
    //         params:
            {
				...
				'ordrqty':$scope.ordrqty,
				'isopencard':$scope.isopencard,
				...
				'type':$scope.type,
				'size':$scope.size,
				'noofcolours':$scope.noofcolours,
				'printapprove':$scope.printapprove,
				'laminationapprove':$scope.laminationapprove,
			//},
    ...
    //         params:
            {
				...
				'jobid':$scope.jobid,
				'jobname':$scope.jobname,
				'cuttingsize':$scope.cuttingsize,
				'slitinput':$scope.slitinput,
				'blcofjob':$scope.blcofjob,
				'noofrolls':$scope.noofrolls,
				'laminationwaste':$scope.laminationwaste,
				'trimwaste':$scope.trimwaste,
