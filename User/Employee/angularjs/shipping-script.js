...
$scope.vm.dtOptions = DTOptionsBuilder.newOptions()
	...
	$scope.FormAdd = false;	
	$scope.FormList = true;		
	$scope.pagetitle = "List of Delivery Challans";
  ...
  $scope.TransportsArr = ['Navata','SRClogic','Diwakar'];
...
$scope.GetGrnId = function()
	{
		$http.get("generate-shipping-id.php")
		.then(function successCallback(response)
		{
			var data = response.data;
			$scope.ShipId = data;
...
$scope.shipcharge = "0.00";
$scope.GetPkg = function(PkgPkId)
{
	if(PkgPkId==undefined || PkgPkId=="")
	{
		$scope.GetListData();
	}
	else
    ...
    $scope.pagetitle="Add Shipping"	
		$scope.GetGrnId();
		$scope.GetPkgInfo(PkgPkId);
	}
}
$scope.GetPkgInfo = function(PkgPkId)
{
	$http.post("Get-Pkg-Info.php",{'PkId': PkgPkId})
		.then(function successCallback(response)
		{
			var data = response.data;			
			if(data=="NoData")
...
$scope.ChangeQty = function(quantity,qtytopack,index)
{
 ...
 //        var InvPkId=new Array();
	...
	// 	}
			...
			//$scope.FileErr="";
      $scope.FileErr="";
      $http.post('add-shipping-process.php', fd, {transformRequest: angular.identity,headers: {'Content-Type': undefined},
            params: {
        ...
        'transporttype':$scope.transporttype,
				...
				'transportinvoiceno':$scope.transportinvoiceno,
 ...
 $scope.SetasDeliver = function(PkId,ShipId,PackageId,InvoiceId)
    {
    ...
    function(isConfirm) {
      if (isConfirm) {
        $http.post('ship-to-deliver-process.php',{'PkId':PkId,'ShipId':ShipId,'PackageId':PackageId,'InvoiceId':InvoiceId,
          }).then(function successCallback(response)
        {
        var data = response.data; 
              if(data=="Success")
