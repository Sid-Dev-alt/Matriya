...
$scope.vm.dtOptions = DTOptionsBuilder.newOptions()
	...
	$scope.pagetitle = "Purchase Receive";
...
$scope.GetGrnId = function()
	{
		$http.get("generate-POreceive-id.php")
		.then(function successCallback(response)
		{
			var data = response.data;
			$scope.ReceiveId = data;
		});
	}
	...
	$scope.GetDueDate = function(entrydate,paymentterms)
{
	...
	$scope.ChkQty = function(AvlQty,quantity,OrderQuantity,index)
	{
		if(Number(quantity)>Number(OrderQuantity))
		{
			$scope.BatchArr[index].quantity = Number(OrderQuantity);	
		}

		if(Number(quantity)>Number(AvlQty))
		{
			$scope.BatchArr[index].quantity = Number(AvlQty);	
		}
...
$scope.GetOrder = function(POrderPkId)
{
	if(POrderPkId==undefined)
	{
		$scope.GetListData();
	}
	else
	{
		...
		$scope.GetInventory();
			...
			$http.post("Get-PO-info.php",{'PkId': POrderPkId})
			.then(function successCallback(response)
			{
				var data = response.data;			
				if(data=="NoData")
				...
				else
				{
					...
					$scope.POrderId =data['POrderId'];
					$scope.OrderDate =data['POrderDate'];
					...
					$scope.vendorid =data['CustomerId_CustomerMaster'];
					$scope.vendorname =data['CustomerName'];
...
$scope.InventoryArray = [];
 ...
 $scope.getDiscAmt = function (sum,disctype,discvalue,additionalcharges,gstperc) 
 {
 	if(disctype=="Percent")
 	{
 		...
 		$scope.totalamount = (sum+$scope.gstpercamt+parseFloat(additionalcharges))-((sum+$scope.gstpercamt+parseFloat(additionalcharges))*(discvalue/100));
 	}
		...
		if($scope.disctype=="Percent")
	 	{
	 		...
	 		$scope.disctotal = -(sum+$scope.gstpercamt+parseFloat($scope.additionalcharges))*(discvalue/100);

	 		$scope.totalamount = sum+$scope.gstpercamt+parseFloat($scope.additionalcharges)-(sum+$scope.gstpercamt+parseFloat($scope.additionalcharges)*($scope.discvalue/100));
	 	}
...
$scope.GotoList = function()
{
	...
	$scope.SaveData = function()
	{
		var fd = new FormData();
          ...
          fd.append('file', file)
		$scope.FileErr="";
			$http.post('add-purchase-receive-process.php', fd, {transformRequest: angular.identity,headers: {'Content-Type': undefined},
            params: {
			...
			.then(function successCallback(response)
			{
				...
				if(data=="Success")
				{
					...
					swal({title: "Added",
							     text: "status changed to received",
							     type: "success",
							     timer: 1000 
						},function () {window.location.href="list-of-pos.php";})
				}
				else
				{
