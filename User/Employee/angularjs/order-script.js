...
AddCategory.controller("CategoryController", function ($scope, $timeout, $http, jsonFilter, DTOptionsBuilder, DTColumnBuilder,DTColumnDefBuilder)
{
	$scope.FormAdd = false;	
	$scope.FormList = true;	
	$scope.pagetitle = "List of Sales Orders";
...
$scope.GetOrderId= function()
	{
		$http.get("generate-order-id.php")
		.then(function successCallback(response)
		{
			var data = response.data;
			$scope.OrderId = data;
...
$scope.InventoryArray = [];
	...
	{
		$http.get("Get-Inventory.php", {responseType:"text/plain"})
		.then(function successCallback(response)
		{
			var data = response.data;  
			if(data=="NoData")
 ...
 $scope.getDiscAmt = function (sum,disctype,discvalue,additionalcharges) 
 {
 	if(disctype=="Percent")
 	{
 		$scope.disctotal = -(sum+parseFloat(additionalcharges))*(discvalue/100);
 		$scope.totalamount = sum+parseFloat(additionalcharges)+parseFloat($scope.disctotal);
 	}
...
$scope.calculateSum = function () {
	...
	if ($scope.BatchArr !== undefined) {
		...
		if($scope.disctype=="Percent")
	 	{
	 		$scope.disctotal = -(sum+parseFloat(additionalcharges))*($scope.discvalue/100);
	 		$scope.totalamount = sum+parseFloat($scope.additionalcharges)+parseFloat($scope.disctotal);
	 	}
