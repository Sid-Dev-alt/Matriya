...
$scope.ChallanTypeArray = [];
$scope.GetOrderId= function()
	{
		$http.get("generate-DC-id.php")
		.then(function successCallback(response)
		{
			var data = response.data;
			$scope.DcId = data;
...
$scope.onInvSelect = function ($item, $model, $label, index, quantity) {
    $scope.$item = $item;
...
// 	}
	$scope.ChallanTypeArray = ['Supply of Liquid gas','Job work','Supply on Approval','Others'];
	...
	for(i=0;i<$scope.BatchArr.length;i++)
	    {
		 ...
		 price.push($scope.BatchArr[i].price);
			...
			$scope.FileErr="";
			$http.post('add-dc-process.php', fd, {transformRequest: angular.identity,headers: {'Content-Type': undefined},
            params: {
				'DcId':$scope.DcId,
