...
else
{
	...
	$query = $dbConnection->prepare("SELECT PM.*,cat.CategoryName FROM ProductMaster AS PM INNER JOIN Category AS cat ON cat.PkId=PM.PkId_Category WHERE PM.DeleteStatus=? ORDER BY PM.PkId DESC LIMIT $first,$itemsPerPage");
	...
	{	
		...
		$sql = $dbConnection->prepare("SELECT COUNT(PkId) AS Total FROM ProductMaster WHERE DeleteStatus=?");
