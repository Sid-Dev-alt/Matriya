...
<html lang="en">
	<head>
    	...
    	<script src="angularjs/size-script.js"></script>
	</head>
									...
									<table id="dynamic-table" class="table table-bordered table-striped table-condensed cf"  datatable="ng" dt-options="vm.dtOptions">
										...
										<tbody >
											<tr  ng-repeat="cat in pagedItems">
												...
												<td data-title="Size">{{cat.Size}}</td>
												<td data-title="Action">
													<button type="button" class="btn btn-xs btn-info" data-ng-click="EditCat(cat.PkId,cat.Size)">Edit &nbsp;
								...
								<!-- PAGE CONTENT BEGINS -->
								 <form class="form-horizontal" role="form" id="AddCategoryForm" name="AddCategoryForm" autocomplete="off" enctype="multipart/form-data" novalidate="" data-ng-submit="AddCategoryData()">
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="categoryname"> Size <span class="error">*</span></label>
										<input type="hidden" value="{{FormPkId}}">
										<div class="col-sm-9">
											<input type="text" id="size" placeholder="" class="col-xs-10 col-sm-5" name="size" required  autofocus data-ng-model="size" ng-change="checkCatgory(FormPkId,size)">
												<div class="error" data-ng-show="submitted || AddCategoryForm.size.$dirty && AddCategoryForm.size.$invalid">
													<small class="error" data-ng-show="AddCategoryForm.size.$error.required">Size is required.</small>
													<small class="error" data-ng-show="AddCategoryForm.size.$error.minlength">Size is required to be at least 3 characters</small>
													<small class="error" data-ng-show="AddCategoryForm.size.$error.pattern">Size should be alphabets ex:abcd</small>
