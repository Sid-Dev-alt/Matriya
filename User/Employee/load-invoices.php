...
{		
	...
	$first = ($pagenumber-1)*$itemsPerPage;
	 ...
	 $query = $dbConnection->prepare("SELECT * FROM Invoices WHERE DeleteStatus=? ORDER BY PkId DESC LIMIT $first,$itemsPerPage");
	...
	{	
		...
		$sql = $dbConnection->prepare("SELECT  COUNT(PkId) AS Total FROM  Invoices WHERE DeleteStatus=?");
